...
void RANSktau::setup(nrs_t *nrsIn, dfloat mueIn, dfloat rhoIn, int ifld, std::string & model)
{
  ...
  o_OiOjSk = platform->device.malloc(nrs->fieldOffset, sizeof(dfloat));
  o_SijMag2 = platform->device.malloc(nrs->fieldOffset, sizeof(dfloat));
  o_xk = platform->device.malloc(cds->fieldOffset[kFieldIndex], sizeof(dfloat));
  o_xt = platform->device.malloc(cds->fieldOffset[kFieldIndex], sizeof(dfloat));
  o_xtq = platform->device.malloc(cds->fieldOffset[kFieldIndex], sizeof(dfloat));
...
{
  o_ywd = platform->device.malloc(nrsIn->fieldOffset, sizeof(dfloat));
  o_ywd.copyFrom(ywd, nrsIn->fieldOffset * sizeof(dfloat));
